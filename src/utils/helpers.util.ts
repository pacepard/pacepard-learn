...
import type { ICountry, IPagination, IDateToday, IHelper } from '@/utils/interfaces.util';
import countries from '../_data/countries.json';
import type { FormatDateType } from '@/utils/types.util';
...
const fitMenus = () => {
    ...
    for (let i = 0; i < boxMenus.length; i++) {
        ...
        var selectBoxSearchInput = $(selectBoxSearch).children(
            '.menu-search__input',
        )[0];
...
const hideMenu = () => {
    ...
    window.onclick = function (e) {
        ...
        for (let j = 0; j < boxMenus.length; j++) {
            ...
            if (control) {
                var singleLabel = $($(control).children()[0]).children(
                    '.single__label',
                )[0];
                var singlePlace = $(
                ...
                var singleImage = $(
                    $($(control).children()[0]).children('.single__image')[0],
                ).children('img')[0];
